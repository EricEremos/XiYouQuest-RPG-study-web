 ...
 * Deno port of src/lib/iflytek-speech/asr-segments.ts (logic must stay identical).
 *
...
export function findQuietSplit(
  ...
  const searchStart = Math.max(
    start + RMS_FRAME_BYTES,
    hardEnd - SPLIT_SEARCH_BYTES,
  );
  ...
  for (
    ...
    frame + RMS_FRAME_BYTES <= hardEnd;
    frame += RMS_FRAME_BYTES
  ) {
